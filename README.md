# Abeeb Babatunde

**Technical Writer & GRC Strategist | Lagos, Nigeria**

I translate complex security frameworks into documentation that non-technical stakeholders can actually understand and act on. My work sits at the intersection of cybersecurity compliance, technical writing, and operational clarity for fintech and B2B SaaS companies.

Currently studying Public Policy at Miva Open University and completing the Google Cybersecurity Professional Certificate.

---

## What You Will Find Here

| Folder | Contents |
|---|---|
| [`/articles`](./articles/) | Published articles on GRC, compliance, and technical writing |
| [`/grc-resources`](./grc-resources/) | Frameworks, templates, and reference material I have developed |

---

## Featured Work

### The CARE Method
A four-step framework for refactoring GRC documentation so the people who need to execute it actually can. Developed and published as part of my HackerNoon writing.

[Read the CARE Method breakdown](./grc-resources/CARE-Method.md)

---

## Published Articles

| Title | Platform | Published |
|---|---|---|
| [How to Write GRC Documentation That Non-Technical Stakeholders Actually Understand](./articles/grc-documentation-non-technical-stakeholders.md) | HackerNoon | March 2026 |
| [What NIST CSF 2.0 Actually Means for a Nigerian Fintech Platform](https://abeebstacks.medium.com/what-nist-csf-2-0-actually-means-for-a-nigerian-fintech-platform-b575413a2e54) | Medium | March 2026 |
| [What to Say When a Series A Investor Asks About Your Cybersecurity Framework](https://abeebstacks.medium.com/what-to-say-when-a-series-a-investor-asks-about-your-cybersecurity-framework-14d03c606ef1) | Medium | March 2026 |

---

## Frameworks I Work With

- NIST Cybersecurity Framework (CSF 1.1 and 2.0)
- ISO 27001
- GDPR
- Nigeria Data Protection Act (NDPA) / NDPC Guidelines
- SOC 2

---

## Connect

- **GitHub:** [github.com/abeebstacks](https://github.com/abeebstacks)
- **HackerNoon:** [hackernoon.com/u/abeebstacks](https://hackernoon.com/u/abeebstacks)
- **LinkedIn:** [linkedin.com/in/abeebstacks](https://www.linkedin.com/in/abeebstacks/)
- **Email:** babsayandayo@email.com

---

*This repository is a living portfolio. It is updated as I publish and build.*
